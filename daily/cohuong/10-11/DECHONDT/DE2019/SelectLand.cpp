...
namespace sub1{
    ...
    void solve(){
        ...
        if ( tmp > eps ) cout << fixed << setprecision(1) << ans;
...
namespace sub2{
    void solve(){
        int chot;
        if ( cross[1].fi == cross[2].fi || cross[1].fi == cross[2].se )
            chot = cross[1].fi;
        else chot = cross[1].se;
        ...
        for (int i = 1; i <= n - 2; i ++){
            if ( i % 2 )
                ans1 += stg(a[chot], a[st-1], a[st]);
            else
                ans2 += stg(a[chot], a[st-1], a[st]);
            st ++;
        }

        ld ans = max(ans1, ans2);
        ld tmp = abs(ans - (ll)ans);
        if ( tmp > eps ) cout << fixed << setprecision(1) << ans;
        else cout << fixed << setprecision(0) << ans;
    }
